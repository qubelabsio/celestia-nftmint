...
export default class Dapp extends React.Component<Props, State> {
    ...
    try {
      ...
      toast.success(
        <>
          NFT Minted!
          <br />
          See <a href="" target="_blank" rel="noopener">here</a> for more details.
          {/* <a
  ...
  async getNFTs(): Promise<void> {
    try {
      ...
      const tokensOfOwner = await this.contract.tokensOfOwner(
        this.state.userAddress!
      );
      const nfts: NftMetadata[] = [];

      const ipfsGateway = "https://ipfs.io/ipfs/";
      const cidBase = "QmdaVqc8zBZzgJ4QBXhi3zjj2y2bHhptu8Z4q4SzJcUVc3";

      for (let i = 0; i < tokensOfOwner.length; i++) {
        const tokenId = tokensOfOwner[i].toNumber();
        const metadataCid = `${cidBase}/${tokenId}.json`;
        const metadataUri = ipfsGateway + metadataCid;
  ...
  async readStream(response: any): Promise<Buffer> {
    const chunks: Buffer[] = [];
      ...
      toast.info(
        <>
          ...
          {/* <a
            ...
            rel="noopener"
          > */}
          View on our Block Explorer <strong>?Coming Soon!</strong>
          {/* </a> */}
        </>
      );
      ...
      toast.success(
        <>
          ...
          {/* <a
            ...
            rel="noopener"
          > */}
          NFT Minted! See <a href="" target="_blank" rel="noopener">here</a> for more details.
          {/* </a> */}
        </>
      );
    ...
    this.setState({
      merkleProofManualAddressFeedbackMessage: (
        <>
          ...
          You can paste it into our Block Explorer <strong>Coming Soon!</strong>
          {/* <a href={this.generateContractUrl()} target="_blank">
            {this.state.networkConfig.blockExplorer.name}
          </a>{" "} */}
        ...
        {this.isWalletConnected() ? (
              ...
              <>
                ...
                {!this.isSoldOut() ? (
                  <>
                    ...
                    />
                    <NftGallery nfts={this.state.nfts} />
                  </>
                ) : (
                  <div className="collection-sold-out">
                    ...
                    You can buy from our beloved holders on . coming soon!
                  </div>
                )}
              </>
            ) : (
          ...
          <div className="no-wallet">
           ...
           <div className="use-block-explorer">
              Welcome to <strong>Celestia NFT Mint</strong>. <span className="emoji">🚀</span><br /><br />
               
              In order to continue, please make sure your Metamask wallet is <strong>
                open</strong> and click on the <strong>Connect Wallet</strong> button above.
              ...
              <br />
              <br />
              Build modular. Be free <span className="emoji">❤️</span>
            </div>          </div>
        )}
      </>
    );
  ...
  private async refreshContractState(): Promise<void> {
    this.setState({
      ...
      metadata: (await this.contract.uriPrefix()).toString(),
  ...
  private async initWallet(): Promise<void> {
    ...
    let networkConfig: NetworkConfigInterface = CollectionConfig.mainnet; // Initialize with default value
    ...
    if (network.chainId === CollectionConfig.mainnet.chainId) {
      ...
      const networkName = "Qubelabs Rollup"; // Replace with the desired network name
      const networkId = "0x2328"; // Replace with the desired network ID
      const rpcUrl = "https://rollup-bsr-celestia.qubelabs.io"; // Replace with the desired RPC URL

      const instructions = `To continue, please add the following network to your MetaMask:
      ...
      if (confirm(instructions)) {
        ...
        // User chose not to add the network, handle accordingly
        return;
      }
    }
